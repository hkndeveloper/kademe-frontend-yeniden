      ...
      <div className="container mx-auto flex h-20 items-center justify-between px-6">
        ...
        <div className="flex items-center gap-4">
          ...
          <button
            ...
            className="flex h-10 w-10 items-center justify-center rounded-xl border border-slate-200 text-slate-600 transition-colors hover:bg-slate-100 md:hidden"
          ...
          <nav className="container mx-auto flex flex-col gap-1 px-6 py-4">
            {navLinks.map((item) => (
              <Link
                ...
                className="rounded-xl px-4 py-3 text-sm font-medium text-slate-700 transition-colors hover:bg-slate-100 hover:text-primary"
              >
                {item.label}
              </Link>
            ))}
              ...
              <div className="ml-4 flex flex-col gap-1 border-l-2 border-primary/20 pl-4">
                {projects.map((p) => (
                  <Link
                    ...
                    href={`/projects/${p.slug}`}
                    onClick={() => setMobileMenuOpen(false)}
                    className="rounded-lg px-3 py-2 text-xs text-slate-500 transition-colors hover:bg-slate-50 hover:text-primary"
                  >
                    {p.name}
                  </Link>
                ))}
            ...
            {/* Mobile Auth Buttons */}
            <div className="mt-3 flex flex-col gap-2 border-t border-slate-200/60 pt-4">
              {isAuthenticated ? (
                <>
                  <Link
                    href={getDashboardLink()}
                    onClick={() => setMobileMenuOpen(false)}
                    className="flex items-center justify-center gap-2 rounded-xl bg-primary px-6 py-3 text-sm font-bold text-primary-foreground shadow-md"
                  >
                    <LayoutDashboard className="h-4 w-4" />
                    Panelim
                  </Link>
                  <button
                    ...
                    className="flex items-center justify-center gap-2 rounded-xl border border-slate-200 px-6 py-3 text-sm font-medium text-red-600 transition-colors hover:bg-red-50"
                  >
                    <LogOut className="h-4 w-4" />
                    Çıkış Yap
                  </button>
                ...
                <>
                  <Link
                    href="/auth/login"
                    onClick={() => setMobileMenuOpen(false)}
                    className="flex items-center justify-center rounded-xl border border-slate-300 px-6 py-3 text-sm font-medium text-slate-800 transition-colors hover:bg-slate-100"
                  >
                    {siteSettings?.navigation.header_login_label || defaultSiteSettings.navigation.header_login_label}
                  </Link>
                  <Link
                    href="/auth/register"
                    onClick={() => setMobileMenuOpen(false)}
                    className="flex items-center justify-center rounded-xl bg-primary px-6 py-3 text-sm font-bold text-primary-foreground shadow-md"
                  >
                    {siteSettings?.navigation.header_register_label || defaultSiteSettings.navigation.header_register_label}
                  </Link>
                </>
