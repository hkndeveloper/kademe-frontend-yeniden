...
export function UnifiedPanelSidebar() {
  ...
  return (
    <aside className="fixed left-0 top-0 z-40 flex h-screen w-72 flex-col border-r border-white/[0.08] bg-gradient-to-b from-slate-900 via-[#0a1020] to-[#05070c] shadow-[inset_-1px_0_0_rgba(255,255,255,0.04)]">
            ...
            {items.map((item) => {
              ...
              return (
                <Link key={item.id} href={item.href} className={cn(navItemBase, isActive ? navActive : navIdle)}>
                  ...
                  {isActive && (
                    <ChevronRight className="h-4 w-4 text-white/90 motion-safe:animate-panel-sidebar-chevron" />
                  )}
                </Link>
              );
            })}
      ...
      <div className="border-t border-white/[0.08] bg-black/15 p-3 backdrop-blur-[2px]">
        <div className="mb-3 flex items-center gap-3 rounded-xl bg-white/[0.05] p-2.5 ring-1 ring-white/[0.06] transition-shadow duration-300 hover:bg-white/[0.07]">
