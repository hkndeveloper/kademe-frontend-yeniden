...
export default function CoordinatorFinancialsPage() {
  ...
  const loadTransactions = useCallback(async () => {
    ...
    try {
      ...
      const response = await api.get("/coordinator/financials", { params });
  ...
  const handleDownload = async (id: number) => {
    try {
      const res = await api.get(`/admin/financials/${id}/invoice`, { responseType: "blob" });
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      await api.post("/coordinator/financials", formData, {
    ...
    <div className="space-y-8">
      <div className="flex flex-col justify-between gap-6 md:flex-row md:items-center">
        ...
        <PermissionGate permission="financial.export">
          <ExportButtons
            endpoint="/coordinator/financials/export"
