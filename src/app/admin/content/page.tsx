...
export default function AdminContentPage() {
  ...
  useEffect(() => {
    const loadContent = async () => {
      try {
        const response = await api.get<ContentResponse>("/admin/content");
  ...
  const handleBlogSave = async () => {
    ...
    try {
      if (editingBlogId) {
        await api.put(`/admin/content/blogs/${editingBlogId}`, {
          ...blogForm,
        ...
        await api.post("/admin/content/blogs", {
          ...blogForm,
          category_id: blogForm.category_id ? Number(blogForm.category_id) : null,
          published_at: blogForm.published_at || null,
        });
  ...
  const handleFaqSave = async () => {
    ...
    try {
      if (editingFaqId) {
        await api.put(`/admin/content/faqs/${editingFaqId}`, faqForm);
        setMessage("SSS maddesi guncellendi.");
      } else {
        await api.post("/admin/content/faqs", faqForm);
      ...
      <div className="flex flex-col justify-between gap-4 md:flex-row md:items-start">
        ...
        <div className="flex flex-wrap gap-3">
          <PermissionGate permission="content.blog.export">
            <ExportButtons endpoint="/admin/content/blogs/export" filename="blog_yazilari" buttonLabel="Bloglari Disa Aktar" />
          </PermissionGate>
          <PermissionGate permission="content.faq.export">
            <ExportButtons endpoint="/admin/content/faqs/export" filename="sss_listesi" buttonLabel="SSSleri Disa Aktar" />
          </PermissionGate>
        </div>
      </div>
