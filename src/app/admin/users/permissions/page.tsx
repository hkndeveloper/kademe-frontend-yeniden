...
import { CheckCircle2, History, Loader2, Plus, Save, Shield, ShieldAlert, UserCog, X } from "lucide-react";
...
export default function PermissionsPage() {
  ...
  const loadAudit = useCallback(async () => {
    ...
    try {
      const response = await api.get<{ logs: PermissionAuditLog[]; warning?: string }>("/admin/permissions-matrix/audit");
  ...
  const loadData = async () => {
    try {
      const [response, userResponse] = await Promise.all([
        api.get<PermissionMatrixResponse>("/admin/permissions-matrix"),
        api.get<{ users: ManagedUser[] }>("/admin/permissions-matrix/users"),
      ]);
  ...
  const loadUserOverrides = useCallback(async (userId: string) => {
    ...
    try {
      const response = await api.get<UserOverrideResponse>(`/admin/permissions-matrix/users/${userId}`);
  ...
  const handleSaveOverrides = async () => {
    ...
    try {
      await api.put(`/admin/permissions-matrix/users/${selectedUserId}`, {
