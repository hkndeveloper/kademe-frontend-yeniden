  ...
  const loadCalendar = useCallback(async () => {
    ...
    try {
      const [overviewResponse, periodResponse] = await Promise.all([
        ...
        api.get<{ periods?: Period[] }>("/admin/periods"),
      ]);
                      ...
                      <div key={program.id} className="rounded-2xl border border-white/5 bg-white/5 p-4">
                        <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                          ...
                          <div className="text-right text-sm text-muted-foreground">
                            {new Date(program.start_at).toLocaleTimeString("tr-TR", {
                              hour: "2-digit",
                              minute: "2-digit",
                            })}
                            {program.end_at
                              ? ` - ${new Date(program.end_at).toLocaleTimeString("tr-TR", {
                                  hour: "2-digit",
                                  minute: "2-digit",
                                })}`
                              : ""}
                        ...
                        <div className="mt-4 flex justify-end">
                          <button
                            ...
                            disabled={
                              !hasPermission("calendar.assignments.manage") ||
                              !canAccessProject("calendar.assignments.manage", program.project_id)
                            }
                            className="inline-flex items-center gap-2 rounded-xl border border-indigo-500/20 bg-indigo-500/10 px-4 py-2 text-xs font-bold uppercase tracking-widest text-indigo-300 transition hover:bg-indigo-500 hover:text-white disabled:cursor-not-allowed disabled:opacity-40"
                          >
