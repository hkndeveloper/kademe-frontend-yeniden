...
export default function AdminPeriodsPage() {
  ...
  const loadData = async () => {
    try {
      const [projectResponse, periodResponse] = await Promise.all([
        ...
        api.get<{ periods: PeriodItem[] }>("/admin/periods"),
      ]);
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      if (editingPeriodId) {
        await api.put(`/admin/periods/${editingPeriodId}`, {
          name: payload.name,
                  ...
                  <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
                    ...
                    <div className="flex flex-wrap gap-2">
                      ...
                      {canAccessProject("projects.application_form.update", period.project_id) ? (
                        <Link
                          href={`/admin/periods/form-builder?project_id=${period.project_id}&period_id=${period.id}`}
