...
export default function AdminFinancialsPage() {
    ...
    try {
      const [financialResponse, projectsResponse] = await Promise.all([
        ...
        hasPermission("projects.view")
          ? api.get<{ projects: Array<{ id: number; name: string }> }>("/admin/projects/manageable")
  ...
  const handleAction = async (id: number, action: "approve" | "reject" | "pay" | "delete") => {
    ...
    try {
      if (action === "delete") {
        await api.delete(`/admin/financials/${id}`);
      } else {
        await api.put(`/admin/financials/${id}/${action}`);
      }
  ...
  const downloadInvoice = async (id: number, name: string) => {
    ...
    try {
      const response = await api.get(`/admin/financials/${id}/invoice`, {
        responseType: "blob",
      });
    ...
    <div className="space-y-10">
      <div className="flex flex-col justify-between gap-6 md:flex-row md:items-center">
        ...
        <PermissionGate permission="financial.export">
          <ExportButtons
            endpoint="/admin/financials/export"
