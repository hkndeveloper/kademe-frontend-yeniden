...
export default function AdminSupportPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const ticketPromise = api.get("/admin/support/tickets", {
        params: {
      ...
      const staffPromise = hasPermission("users.view")
        ...
        : hasPermission("support.assign")
          ? api.get<{ users: StaffUser[] }>("/admin/support/assignable-users")
  ...
  const handleAssign = async (ticketId: number) => {
    ...
    try {
      await api.put(`/admin/support/tickets/${ticketId}/assign`, {
