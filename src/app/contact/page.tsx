    ...
    <div className="min-h-screen bg-background pb-24">
      ...
      <div className="container mx-auto mt-20 grid grid-cols-1 gap-12 px-6 lg:grid-cols-3">
        <div className="space-y-8 lg:col-span-1">
          <div className="glass-panel space-y-10 rounded-3xl p-8">
        ...
        <div className="lg:col-span-2">
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="glass-panel rounded-[40px] p-8 md:p-12">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
                <div className="space-y-2">
                  ...
                  <input required type="text" value={form.name} onChange={(event) => handleChange("name", event.target.value)} className="w-full rounded-2xl border border-border bg-input px-6 py-4 outline-none transition-all focus:ring-2 focus:ring-primary" placeholder="Adiniz Soyadiniz" />
                </div>
                <div className="space-y-2">
                  <label className="ml-2 text-xs font-bold uppercase tracking-widest text-muted-foreground">E-posta</label>
                  <input required type="email" value={form.email} onChange={(event) => handleChange("email", event.target.value)} className="w-full rounded-2xl border border-border bg-input px-6 py-4 outline-none transition-all focus:ring-2 focus:ring-primary" placeholder="email@ornek.com" />
                </div>
              </div>

              <div className="space-y-2">
                <label className="ml-2 text-xs font-bold uppercase tracking-widest text-muted-foreground">Baslik</label>
                <input type="text" value={form.subject} onChange={(event) => handleChange("subject", event.target.value)} className="w-full rounded-2xl border border-border bg-input px-6 py-4 outline-none transition-all focus:ring-2 focus:ring-primary" placeholder="Mesaj basligi" />
              </div>

              <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
                <div className="space-y-2">
                  <label className="ml-2 text-xs font-bold uppercase tracking-widest text-muted-foreground">Konu</label>
                  <select value={form.category} onChange={(event) => handleChange("category", event.target.value)} className="w-full appearance-none rounded-2xl border border-border bg-input px-6 py-4 outline-none transition-all focus:ring-2 focus:ring-primary">
                ...
                <div className="space-y-2">
                  ...
                  <select value={form.project_id} onChange={(event) => handleChange("project_id", event.target.value)} className="w-full appearance-none rounded-2xl border border-border bg-input px-6 py-4 outline-none transition-all focus:ring-2 focus:ring-primary">
              ...
              <div className="space-y-2">
                ...
                <textarea required rows={5} value={form.message} onChange={(event) => handleChange("message", event.target.value)} className="w-full resize-none rounded-2xl border border-border bg-input px-6 py-4 outline-none transition-all focus:ring-2 focus:ring-primary" placeholder="Size nasil yardimci olabiliriz?" />
              </div>

              <button type="submit" disabled={loading} className="flex w-full items-center justify-center gap-3 rounded-2xl bg-primary py-5 font-bold text-primary-foreground shadow-md transition-all hover:scale-[1.02] active:scale-95 disabled:cursor-not-allowed disabled:opacity-70">
