...
export default function AlumniAssignmentsPage() {
  ...
  useEffect(() => {
    const fetchAssignments = async () => {
      try {
        ...
        console.error("Mezun odevleri cekilemedi", error);
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleSubmit = async (assignmentId: number) => {
    ...
    try {
      ...
      console.error("Mezun teslimi basarisiz", error);
    } finally {
      setSubmitting(null);
    }
  };
    ...
    <div className="space-y-8">
      <div className="flex flex-col justify-between gap-5 lg:flex-row lg:items-center">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-black text-slate-900">Odevlerim ve Gorevlerim</h1>
            <p className="mt-1 text-sm font-bold uppercase tracking-widest text-muted-foreground">Mezuniyet sonrasi ve gecmis teslim kayitlari</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 gap-6">
        {assignments.length === 0 ? (
          <div className="glass-panel rounded-3xl border border-dashed border-border p-20 text-center text-muted-foreground">
            ...
            Henuz atanmis bir odev veya gorev gorunmuyor.
          </div>
        ) : (
                ...
                <div className="grid gap-0 lg:grid-cols-[1fr,220px]">
                  ...
                  <div className="flex flex-col justify-between border-t border-border bg-background/50 p-6 lg:border-l lg:border-t-0">
                    ...
                    >
                      ...
                      {submitted ? "Teslimi Guncelle" : "Teslim Et"}
                    </button>
                  </div>
                </div>
