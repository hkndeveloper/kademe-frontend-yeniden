...
export default function StaffRequestsPage() {
  ...
  useEffect(() => {
    const loadRequests = async () => {
      try {
        ...
        console.error("Personel talep verileri yuklenemedi", error);
  ...
  const assignedRequests = useMemo(
    () => requests.filter((request) => request.target_user !== null),
    [requests],
  );
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      console.error("Personel talebi olusturulamadi", error);
    ...
    <div className="space-y-8">
      <div className="flex flex-col justify-between gap-6 md:flex-row md:items-center">
        ...
        <PermissionGate permission="requests.export">
          <ExportButtons endpoint="/requests/export" filename="personel_talepler" buttonLabel="Talepleri Disa Aktar" />
        </PermissionGate>
      </div>
      ...
      <div className="grid grid-cols-1 gap-8 lg:grid-cols-[1fr_1fr]">
        ...
        <form className="glass-panel rounded-3xl p-8" onSubmit={handleSubmit}>
          <h2 className="mb-4 text-lg font-bold text-slate-900">Yeni Talep</h2>
          ...
          <button type="submit" disabled={saving} className="mt-6 inline-flex items-center gap-2 rounded-2xl bg-amber-500 px-6 py-3 font-bold text-black disabled:opacity-60">
        ...
        <div className="glass-panel rounded-3xl p-8">
          <h2 className="mb-4 text-lg font-bold text-slate-900">Bana Atanan veya Sectigim Talepler</h2>
          {loading ? (
            <div className="flex min-h-32 items-center justify-center"><Loader2 className="h-6 w-6 animate-spin text-amber-500" /></div>
          ) : assignedRequests.length === 0 ? (
            <div className="text-sm text-muted-foreground">Hedef kullanicisi olan talep kaydi yok.</div>
          ) : (
            <div className="space-y-3">
              {assignedRequests.slice(0, 8).map((request) => (
                <div key={request.id} className="rounded-2xl border border-white/5 bg-white/5 p-4">
                  ...
                  <div className="mt-2 text-xs text-muted-foreground">{request.target_user ? `${request.target_user.name} ${request.target_user.surname}` : "Hedef kisi yok"}</div>
                  ...
                  <div className="mt-4 border-t border-white/5 pt-3">
                    {request.response_file_path ? (
                      <a 
                        ...
                        className="flex w-full items-center justify-center gap-2 rounded-xl bg-amber-500/20 px-3 py-2 text-xs font-bold text-amber-500 hover:bg-amber-500 hover:text-black transition-colors"
                      >
