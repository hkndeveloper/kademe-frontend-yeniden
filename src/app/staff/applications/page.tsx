...
export default function StaffApplicationsPage() {
    ...
    const fetchData = async () => {
      try {
        const [appRes, projRes] = await Promise.all([
          api.get<{ applications: { data: Application[] } }>("/staff/applications"), // Backend filters by staff-visible projects
  ...
  const handleStatusChange = async (id: number, newStatus: string) => {
    try {
      await api.put(`/staff/applications/${id}/status`, { status: newStatus });
  ...
  return (
    ...
    <div className="space-y-8">
      <div className="flex flex-col justify-between gap-6 md:flex-row md:items-center">
        ...
        >
        <ExportButtons
          endpoint="/staff/applications/export"
